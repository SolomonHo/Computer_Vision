...
def plot_learning_curve(
    ...
    train_acc = to_numpy(result_lists['train_acc'])
    train_loss = to_numpy(result_lists['train_loss'])
    val_acc = to_numpy(result_lists['val_acc'])
    val_loss = to_numpy(result_lists['val_loss'])
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                ...
                if batch % 100 == 0:
                    print(f'[{epoch + 1}/{cfg.epochs}] Val batch: {batch + 1} / {len(val_loader)}')
                # Data loading.
                images, labels = data['images'].to(device), data['labels'].to(device)  
